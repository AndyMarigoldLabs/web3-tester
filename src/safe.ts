...
export type SafeTransactionService = {
  ...
  confirmTransaction(
    safeTxHash: Hex,
    confirmation: SafeTransactionConfirmation,
  ...
  markExecuted?(
    safeTxHash: Hex,
    execution: { transactionHash: Hex; executorAddress: Address },
  ): Promise<SafeMultisigTransaction>;
};
...
export type SafeAppBridgeOptions = {
  ...
  addressBook?: readonly { address: Address; chainId: string; name: string }[];
...
export const SAFE_MULTISEND_CALL_ONLY_ADDRESS =
  '0x9641d764fc13c8b624c04430c7356c1c7c8102e2' as const;
const SAFE_MULTISEND_SELECTOR = keccak256(toHex('multiSend(bytes)')).slice(0, 10) as Hex;
...
const assertAddress = (value: string, label: string): Address => {
  if (!isAddress(value)) {
...
const decimal = (value: bigint | number | string | undefined, fallback = 0n): string => {
...
const bigintValue = (value: string): bigint => BigInt(value);

export function normalizeSafeTransactionData(
  data: SafeTransactionData,
  ...
  return {
    ...
    refundReceiver: assertAddress(
      data.refundReceiver ?? ZERO_ADDRESS,
      'Safe transaction refundReceiver',
    ),
    nonce: decimal(data.nonce),
  };
}

export function buildSafeTransactionTypedData(
  safeAddress: Address,
  chainId: number | bigint,
  data: SafeTransactionData,
 ...
 */
export function hashSafeTransactionData(
  safeAddress: Address,
  chainId: number | bigint,
  data: SafeTransactionData,
): Hex {
...
export function deterministicSafeSignature(safeTxHash: Hex, owner: Address): Hex {
  const r = keccak256(
    encodeAbiParameters([{ type: 'bytes32' }, { type: 'address' }, { type: 'string' }], [
      safeTxHash,
      owner,
      'r',
    ]),
...
const normalizeConfirmation = (input: Record<string, unknown>): SafeTransactionConfirmation => ({
...
const normalizeServiceTransaction = (
  input: Record<string, unknown>,
  fallback?: { safeAddress?: Address; senderAddress?: Address },
  ...
  const data = normalizeSafeTransactionData({
    ...
    operation: Number(input.operation ?? 0) as SafeOperation,
    ...
    refundReceiver: assertAddress(
      String(input.refundReceiver ?? ZERO_ADDRESS),
      'Safe transaction refundReceiver',
    ),
  ...
  return {
    ...data,
    safeAddress: assertAddress(
      String(input.safe ?? input.safeAddress ?? fallback?.safeAddress ?? ZERO_ADDRESS),
      'Safe transaction safeAddress',
    ),
    safeTxHash: assertHex(
      String(input.safeTxHash ?? input.contractTransactionHash),
      'Safe transaction safeTxHash',
    ),
    senderAddress: assertAddress(
      String(input.sender ?? input.senderAddress ?? fallback?.senderAddress ?? ZERO_ADDRESS),
      'Safe transaction senderAddress',
    ),
    ...(input.origin ? { origin: String(input.origin) } : {}),
    confirmations: Array.isArray(input.confirmations)
      ? input.confirmations.map((item) => normalizeConfirmation(item as Record<string, unknown>))
      : [],
...
const hasResponseBody = (body: Record<string, unknown> | undefined): body is Record<string, unknown> =>
  body !== undefined && Object.keys(body).length > 0;

export class SafeTransactionServiceClient implements SafeTransactionService {
  private readonly fetchImpl: typeof fetch;
  private readonly baseUrl: string;
  private readonly apiPrefix: string;
  private readonly headers: Record<string, string>;
  private readonly chainId?: number | bigint;
  private readonly safeTxHashStrategy: SafeTransactionHashStrategy;
  ...
  async proposeTransaction(proposal: SafeTransactionProposal): Promise<SafeMultisigTransaction> {
  ...
  async getTransaction(safeTxHash: Hex): Promise<SafeMultisigTransaction> {
    const body = await this.request<Record<string, unknown>>(
  ...
  async listTransactions(safeAddress: Address): Promise<SafeMultisigTransaction[]> {
    const body = await this.request<{ results?: unknown[] }>(
      `/safes/${safeAddress}/multisig-transactions/`,
      { method: 'GET' },
    );
    return (body?.results ?? []).map((item) =>
      normalizeServiceTransaction(item as Record<string, unknown>, { safeAddress }),
    );
  }

  async listConfirmations(safeTxHash: Hex): Promise<SafeTransactionConfirmation[]> {
    const body = await this.request<{ results?: unknown[] }>(
      `/multisig-transactions/${safeTxHash}/confirmations/`,
      { method: 'GET' },
    );
    return (body?.results ?? []).map((item) =>
      normalizeConfirmation(item as Record<string, unknown>),
    );
  }
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      throw new Error(
        `Safe Transaction Service ${options.method} ${path} failed with HTTP ${response.status}: ${text.slice(0, 500)}`,
      );
    }
  ...
  private hashSafeTransaction(safeAddress: Address, data: SafeTransactionData): Hex {
    if (this.safeTxHashStrategy === 'eip712') {
      if (this.chainId === undefined) {
        throw new Error(
          'SafeTransactionServiceClient requires chainId to compute EIP-712 safeTxHash. Pass safeTxHash explicitly or configure chainId.',
        );
      }
...
export class InMemorySafeTransactionService implements SafeTransactionService {
  private readonly transactions = new Map<Hex, SafeMultisigTransaction>();

  async proposeTransaction(proposal: SafeTransactionProposal): Promise<SafeMultisigTransaction> {
    const safeAddress = assertAddress(proposal.safeAddress, 'Safe address');
    const senderAddress = assertAddress(proposal.senderAddress, 'Safe senderAddress');
    const safeTxHash =
      proposal.safeTxHash ?? hashSafeTransactionData(safeAddress, 0n, proposal.data);
    const existing = this.transactions.get(safeTxHash);
    if (existing) {
      throw new Error(`Safe transaction "${safeTxHash}" already exists.`);
    }

    const record: SafeMultisigTransaction = {
  ...
  private mustGet(safeTxHash: Hex): SafeMultisigTransaction {
...
export class SafeWalletHarness {
  ...
  constructor(options: SafeWalletHarnessOptions) {
    ...
    if (!Number.isInteger(this.threshold) || this.threshold < 1 || this.threshold > this.owners.length) {
      throw new Error(
        `Safe threshold must be between 1 and owner count (${this.owners.length}), got ${this.threshold}.`,
      );
    }
  }

  async proposeTransaction(options: {
    transaction: Omit<SafeTransactionData, 'nonce'> & { nonce?: SafeTransactionData['nonce'] };
    proposer?: Address;
    signature?: Hex;
    origin?: string;
  }): Promise<SafeMultisigTransaction> {
    ...
    let allocatedNonce: string | undefined;
    ...
    try {
      ...
      if (allocatedNonce !== undefined) this.rollbackNonce(allocatedNonce);
      throw error;
    }
  }
  ...
  async confirmTransaction(
    safeTxHash: Hex,
    options: { owner: Address; signature?: Hex },
  ): Promise<SafeMultisigTransaction> {
  ...
  async executeTransaction(
    safeTxHash: Hex,
    options: { executor?: Address } = {},
  ): Promise<{ txHash?: Hex; transaction: SafeMultisigTransaction }> {
    ...
    if (transaction.confirmations.length < this.threshold) {
      throw new Error(
        `Safe transaction "${safeTxHash}" has ${transaction.confirmations.length}/${this.threshold} confirmations.`,
      );
    }
    ...
    if (this.rpcClient) {
      ...
      } satisfies JsonRpcRequest)) as Hex;
    }

    const executed =
      txHash && this.transactionService.markExecuted
  ...
  async listTransactions(): Promise<SafeMultisigTransaction[]> {
  ...
  async requestRpc(request: JsonRpcRequest): Promise<unknown> {
    if (!this.rpcClient) {
  ...
  private rollbackNonce(value: string): void {
  ...
  private hashSafeTransaction(data: SafeTransactionData): Hex {
  ...
  private assertOwner(owner: Address): void {
...
export async function handleSafeAppRequest(
  safe: SafeWalletHarness,
  request: SafeAppSdkRequest,
  options: SafeAppBridgeOptions & { origin?: string } = {},
): Promise<unknown> {
  ...
  switch (request.method) {
    case 'getSafeInfo':
      return {
        ...
        modules:
          options.safeInfo?.modules === undefined
            ? null
    ...
    case 'sendTransactions': {
      ...
      const safeTxGas = asRecord(params.params).safeTxGas as number | string | undefined;
      const transaction =
        normalizedTxs.length === 1
          ? {
          ...
          : {
              to: assertAddress(
                options.multiSendAddress ?? SAFE_MULTISEND_CALL_ONLY_ADDRESS,
                'Safe Apps multiSendAddress',
              ),
              value: '0',
    ...
    case 'signMessage': {
      ...
      const messageHash = keccak256(
        encodeAbiParameters([{ type: 'string' }], [message]),
      );
      return {
        messageHash,
    ...
    case 'signTypedMessage': {
      const params = asRecord(request.params);
      const messageHash = keccak256(
        encodeAbiParameters([{ type: 'string' }], [JSON.stringify(params.typedData ?? {})]),
      );
      return {
        messageHash,
        signature: deterministicSafeSignature(messageHash, safe.owners[0]!),
...
export async function injectSafeAppBridge(
  page: Page,
  safe: SafeWalletHarness,
  options: SafeAppBridgeOptions = {},
): Promise<void> {
  const context = page.context();
  try {
    await context.exposeBinding(
      SAFE_APP_BRIDGE_BINDING,
      async (
        _source,
        payload: { request: SafeAppSdkRequest; origin?: string },
      ): Promise<unknown> => {
    ...
    if (error instanceof Error && /already registered/i.test(error.message)) {
      throw new Error(
        'A Safe App bridge is already injected into this browser context. Create one bridge per context.',
        { cause: error },
      );
    }
    throw error;
  ...
  await Promise.all(
    context.pages().map((target) => target.evaluate(script).catch(() => undefined)),
  );
}

export function buildSafeAppBridgeScript(version = '1.0.0'): string {
  return `
(() => {
...
const cloneTransaction = (transaction: SafeMultisigTransaction): SafeMultisigTransaction => ({
  ...transaction,
...
const normalizeSafeAppTransaction = (value: unknown): {
  to: Address;
  value: string;
  data: Hex;
} => {
  ...
  return {
    ...
    value: decimal(tx.value as string | number | bigint | undefined),
    data: assertHex(String(tx.data ?? '0x'), 'Safe App transaction data'),
  };
};

const hexByteLength = (hex: Hex): bigint => BigInt((hex.length - 2) / 2);

const encodeSafeMultiSendTransactions = (
  transactions: readonly ReturnType<typeof normalizeSafeAppTransaction>[],
): Hex => {
...
const encodeSafeMultiSendCall = (
  ...
  return `${SAFE_MULTISEND_SELECTOR}${encodedArgument.slice(2)}` as Hex;
};

const normalizeSafeAppBalances = (
  balances: SafeAppBridgeOptions['balances'],
): SafeAppBalances => {
...
const isSafeAppBalances = (
  balances: SafeAppBridgeOptions['balances'],
): balances is SafeAppBalances =>
  Boolean(
    balances &&
...
const safeTransactionToGatewayDetails = (transaction: SafeMultisigTransaction) => ({
...
const toOrigin = (urlOrOrigin: string): string => {
  try {
