...
import { buildWalletExtensionProfile, cloneWalletProfile, } from './real-wallet-cache.js';
import { benchmarkForTest, benchmarkObjectMethods } from './benchmark.js';
import { launchRealWalletExtension, } from './real-wallet-extension.js';
const extensionPathFromEnv = () => process.env.WEB3_TESTER_REAL_WALLET_EXTENSION_PATH || undefined;
const profileDirFromEnv = () => process.env.WEB3_TESTER_REAL_WALLET_PROFILE_DIR || undefined;
const assertChromiumProject = (browserName) => {
    if (browserName !== 'chromium') {
        throw new Error('@marigoldlabs/web3-tester/real-wallet-extension-fixtures launch a Chromium extension context. ' +
...
export const test = base.extend({
    ...
    realWalletExtension: async ({ realWalletExtensionOptions, browserName }, use, testInfo) => {
        ...
        try {
            ...
            if (!extensionPath) {
                throw new Error('Real-wallet extension fixtures need extensionPath or ' +
            ...
            if (!profileDir) {
                if (!options.profileCacheKey) {
                    throw new Error('Real-wallet extension fixtures need profileCacheKey to build a cached profile, ' +
                        'or an explicit profileDir pointing at a prepared wallet profile.');
                }
                const profileCacheKey = options.profileCacheKey;
                const cachedProfile = await benchmark.measure('realWalletExtension.buildProfile', () => buildWalletExtensionProfile({
                ...
                profileDir = await benchmark.measure('realWalletExtension.cloneProfile', () => cloneWalletProfile(cachedProfile, path.join(testInfo.outputDir, 'wallet-extension-profile')));
            }
            session = await benchmark.measure('realWalletExtension.launch', () => launchRealWalletExtension({
                baseURL: options.baseURL,
                extensionId: options.extensionId,
                extensionName: options.extensionName,
                extensionPath,
...
//# sourceMappingURL=real-wallet-extension-fixtures.js.map