...
const OZ_V5_ERC20_NAMESPACE_ROOT = 0x52c63247e1f47db19d5ce0460030c497f067ca4cebf71ba98eeadabe20bace00n;
const pad32 = (value) => toHex(value, { size: 32 });
const mappingSlot = (layout, holder, base) => layout === 'solidity'
...
const balanceOfData = (holder) => encodeFunctionData({ abi: erc20Abi, functionName: 'balanceOf', args: [holder] });
const totalSupplyData = () => encodeFunctionData({ abi: erc20Abi, functionName: 'totalSupply' });
...
const callUint = async (client, to, data, override) => {
...
const getStorage = async (client, address, slot) => {
...
const setStorage = async (client, token, address, slot, value) => {
    try {
        ...
        if (/not found|not supported|not available|does not exist|unknown method|unsupported/i.test(message)) {
            throw new Erc20DealError('dealErc20 requires an anvil-backed client (the RPC rejected anvil_setStorageAt) — live chains cannot be dealt.', token, 0);
        }
        throw error;
    }
};
const probeSentinelFor = (current) => current === PROBE_SENTINEL ? PROBE_SENTINEL - 1n : PROBE_SENTINEL;
export async function discoverErc20BalanceSlot(client, token, holder, options = {}) {
    ...
    if (current === undefined) {
        throw new Erc20DealError(`${token} does not implement balanceOf — not an ERC-20?`, token, 0);
    }
    ...
    for (let start = 0; start < candidates.length; start += BATCH) {
        const hits = await Promise.all(candidates.slice(start, start + BATCH).map(async (candidate) => {
    ...
    throw new Erc20DealError(`Could not find ${token}'s balance slot after probing ${candidates.length} candidates. ` +
        ...
        'external-storage proxies need { storageAddress }; deep layouts may need a higher { maxSlot }.', token, candidates.length);
}
export async function getErc20Balance(client, token, account) {
...
export async function dealErc20(client, token, account, amount, options = {}, cache) {
    ...
    const explicit = options.slot !== undefined
    ...
    const writeAndVerify = async (slotInfo) => {
    ...
    if (await writeAndVerify(info)) {
        ...
        if (explicit) {
            throw new Erc20DealError(`balanceOf does not reflect a write at the explicit slot — wrong { slot }/{ layout }, or ${token} computes balanceOf.`, token, 0);
        }
        if (freshlyDiscovered) {
            throw new Erc20DealError(`Discovery matched a slot but the final write did not verify — ${token} likely computes balanceOf (shares/rebasing).`, token, 0);
        }
        ...
        if (!(await writeAndVerify(rediscovered))) {
            throw new Erc20DealError(`Verification failed even after rediscovery — ${token} likely computes balanceOf (shares/rebasing).`, token, 0);
        }
...
const adjustTotalSupply = async (client, token, storageAddress, info, delta, context) => {
    if (delta === 0n) {
        return;
    }
    ...
    if (newSupply < 0n) {
        throw new Erc20DealError(`adjustTotalSupply would underflow: totalSupply ${currentSupply} with delta ${delta} (forge-std deal reverts here too).`, token, 0);
    }
    if (newSupply > MAX_UINT256) {
        throw new Erc20DealError(`adjustTotalSupply would overflow uint256: totalSupply ${currentSupply} with delta ${delta}.`, token, 0);
    }
    ...
    if ((await callUint(client, token, data)) !== newSupply) {
        ...
        throw new Erc20DealError(`${token}'s totalSupply did not reflect the adjustment — it is likely computed.`, token, 0);
    }
};
// Plain (unkeyed) slot discovery for totalSupply, also via state overrides.
const discoverPlainSlot = async (client, token, storageAddress, data, current, info, maxSlot) => {
    const sentinel = probeSentinelFor(current);
    const candidates = [];
    ...
    throw new Erc20DealError(`Could not find ${token}'s totalSupply slot after probing ${candidates.length} candidates.`, token, candidates.length);
};
